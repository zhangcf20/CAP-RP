...
int main (int argc, char **argv) {
  ...
  int	repeat = 0;
  int   hi,hj,hk,index;
  char	dst0[10],dst[10],tmp[280],tmp1[280],path[280],*c_pt;
...
/*** design bandpass filter, same as in sac: bp c ${bp1} ${bp2} n 4 p 1 ***/
...
/*    printf("nup(sum of weights for each sta)=%d\n",nup); */
    scanf("%f%f%f",&x,&x1,&x2);
...
/*    if (dmin<0.) dmin=distance;*/
    for(spt=obs->com,kc=2,j=0;j<NCP;j++,spt++,kc=3) {
      ...
      if (j<3) {if (f1_sw>0.)  apply(spt->rec,(long int) npt,0,sw_sn,sw_sd,nsects);}
      else     {if (f1_pnl>0.) apply(spt->rec,(long int) npt,0,pnl_sn,pnl_sd,nsects);}
...
//      taper(spt->rec,npt);  
	...
	*f_pt *= weight;
	x2+=(*f_pt)*(*f_pt);
      ...
      spt->rec2 = x2;
   /*read GF into spt->syn. syn[0,1] for love wave;syn[0,1,2]for body and rayleigh wave */
   /* for Love Wave(tang.) XXX.XX.grn.0/1*/
   /* for Rayleigh Wave(vertical) XXX.XX.grn.2/3/4  */
   /* for Rayleigh Wave(radial) XXX.XX.grn.5/6/7   */
   /* for Pnl Wave(vertical) XXX.XX.grn.2/3/4     */
   /* for Pnl Wave(radial) XXX.XX.grn.5/6/7      */
      for(m=0,k=0;k<kc;k++) {
...
//	taper(f_pt,npt);
	for(l=0; l<npt; l++) f_pt[l] *= weight;
        /*crs-coe of data with GFs,for Love-wave n_GF=2,else n_GF=3*/
...
/*  if (sol.meca.dip>90.) {
  ...
  for(hi=0;hi<grid.n[2];hi++) {
    ...
    /*       solution = (solution-grid.err[sol.others[0]])/x2/dof*100; */
     /*      solution = (solution-sol.err)/sol.err*100; /rescaling/*/
  ...
  for(i_rak=0; i_rak<grid.n[2]; i_rak++) {
     ...
     for(i_dip=0; i_dip<grid.n[1]; i_dip++) {
	  ...
	  for(obs=obs0,sol.err=0.,i=0;i<nda;i++,obs++){
	    ...
	    z_pt  = spt->crl[2];
	    z2 = spt->on_off>0?1:0;
	    for(y1=-FLT_MAX,l=0;l<=max_shft[1];l++) {
	      x =rad[3]*(*f_pt0++)+rad[4]*(*f_pt1++);
              y = z0*x;
              if (y>y1) {y1=y;cfg[0]=x;sol.shft[i][0]=l;}
	      ...
	      x1=rad[0]*(*r_pt0++)+rad[1]*(*r_pt1++)+rad[2]*(*r_pt++);
              y = z1*x1;
              if (y>y1) {y1=y;cfg[1]=x1;sol.shft[i][1]=l;}
	      ...
	      x2=rad[0]*(*z_pt0++)+rad[1]*(*z_pt1++)+rad[2]*(*z_pt++);
              y = z2*x2;
              if (y>y1) {y1=y;cfg[2]=x2;sol.shft[i][2]=l;}
...
float *trap(float t1, float t2, float dt, int *n) {
    ...
    *n = n1+n2;
    s = (float *) malloc((*n+2)*sizeof(float));
    slope = 1./(n1*n2);
    s[0] = 0;
