...
work_folder = '/media/cfz/23z-5/gcap_inversion/updated/fk';
...
par.T_t1 = 40;       par.P_p1 = 0.3;    par.M_m1 = mname;   par.I_i1 = 1;                       par.R_r5 = -90;
...
for count_loop = 1:5
    ...
    if count_loop > 1
        ...
        fprintf(fileID, '%d    %f  %f  %f  %f %f %f\n', count_loop, mt(1,1),mt(1,2),mt(1,3),mt(2,2),mt(2,3),mt(3,3));
        fclose(fileID);

    end
    
%     mt_=[-0.997666161034734; -0.065858177329262;  0.009605007454687;  0.990382403713175;  0.086292412882417;  0.007283757321559];
%     mt(1,1) = mt_(1,1);mt(1,2) = mt_(2,1);mt(1,3) = mt_(3,1);
%     mt(2,2) = mt_(4,1);mt(2,3) = mt_(5,1);
%     mt(3,3) = mt_(6,1);
%     mt(2,1) = mt_(2,1);mt(3,1) = mt_(3,1);mt(3,2) = mt_(5,1);
%     base_a = update_a(az,mt);
